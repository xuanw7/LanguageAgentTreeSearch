...
logging.basicConfig(filename='cr_it.log', level=logging.INFO, format='%(asctime)s - %(levelname)s - %(message)s', filemode='w')
...
def cr_search(args, task, idx, depth_limit=7, to_print=True):
    global gpt
    ...
    # stack = [root]
    records = [root]
    while records and it < iterations:
        p = np.zeros_like(records, dtype='float64')
        if it < 1 / 2 * iterations:
            if len(records) > 1:
        ...
        node = np.random.choice(records, p=p)
        ...
        records.extend(reversed(node.children))  # adding all child nodes to stack for DFS
...
def expand_node(node, args, task):
    ...
    node.children.extend(new_nodes[:1])
